...
CLASS zcl_xlom__ex_fu_vlookup DEFINITION
  ...
  PUBLIC SECTION.
    INTERFACES zif_xlom__ex DATA VALUES name = 'VLOOKUP'
                                        type = zif_xlom__ex=>c_type-function-vlookup.
...
CLASS zcl_xlom__ex_fu_vlookup IMPLEMENTATION.
  METHOD adjust_evaluated_operands.
    ...
    " because {"a","b"} is an array (one or the other, but not both)
        ...
        IF col_index_num < 1.
          RAISE EXCEPTION TYPE zcx_xlom__va
            EXPORTING result_error = zcl_xlom__va_error=>value_cannot_be_calculated
                      text         = 'The column index is too small'.
        ENDIF.

        DATA(optimized_table_array) = zcl_xlom__ut=>intersect_used_range( table_array )-inside_used_range.
