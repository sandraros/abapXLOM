...
CLASS zcl_xlom__ex_op_plus_unary DEFINITION
  PUBLIC FINAL
  CREATE PRIVATE
  ...
  PUBLIC SECTION.
    INTERFACES zif_xlom__ex DATA VALUES name = '+'
                                        type = zif_xlom__ex=>c_type-operation-plus_unary.

    CLASS-METHODS class_constructor.

    CLASS-METHODS create
      IMPORTING operand       TYPE REF TO zif_xlom__ex
      RETURNING VALUE(result) TYPE REF TO zcl_xlom__ex_op_plus_unary.
...
CLASS zcl_xlom__ex_op_plus_unary IMPLEMENTATION.
...
*    zif_xlom__ex~type = zif_xlom__ex=>c_type-operation-minus_unary.
  ...
  METHOD create.
    result = NEW zcl_xlom__ex_op_plus_unary( ).
